...
"""API endpoints for the Occasion (event invitation / check-in) feature.

Ported from the `occasion_invite` branch of the legacy single-account
frappe_whatsapp app onto this app's structure.

Endpoints are available at:
https://your-site.com/api/method/frappe_whatsapp.frappe_whatsapp.api.occasion.<name>
"""
...
def check_in(gate: str, checkin_by: str, qr_code: str = None, invitee_id: str = None):
    ...
    if gate:
        gate_doc = frappe.db.get_value(
            "Occasion Gate Checkin", gate, ["name", "active"], as_dict=True
        )
    ...
    try:
        ...
        frappe.db.set_value(
            "Occasion Invitee",
            invitee.name,
            {"checkin_count": row.checkin_count + 1, "last_checkin": now()},
        )
...
def get_invitee_details(invitee_id):
    try:
        invitee_details = frappe.db.get_value(
            "Occasion Invitee",
            {"name": invitee_id},
            [
                "name",
        ...
        gen_response(200, 0, _("Invitee details fetched successfully"), invitee_details)
    except Exception as e:
        gen_response(500, 1, _("Internal server error: {0}").format(str(e)), [])
